...
// ── SQLite: better-sqlite3 (npm, любой Node) или node:sqlite (Node 22.5+) ──
let db;
let useSQL = false;

// Загружаем SQLite: сначала better-sqlite3, потом node:sqlite
function loadSQLite() {
  try {
    const BetterSQLite3 = require('better-sqlite3');
    const instance = new BetterSQLite3(DB_FILE);
    console.log('[DB] better-sqlite3 подключена ✓');
    return instance;
  } catch (e1) {
    try {
      const { DatabaseSync } = require('node:sqlite');
      const raw = new DatabaseSync(DB_FILE);
      // Оборачиваем node:sqlite в совместимый с better-sqlite3 интерфейс
      return {
        _raw: raw,
        ...
        prepare(sql) {
          ...
          return {
            run(...a) { return s.run(...a); },
            get(...a) { return s.get(...a); },
            all(...a) { return s.all(...a); },
          };
        },
        close() { if (raw.close) raw.close(); },
      };
    } catch (e2) {
      throw new Error('better-sqlite3: ' + e1.message + ' | node:sqlite: ' + e2.message);
    }
  }
}
...
async function handle(req, res) {
  ...
  if (pathname === '/events') {
    ...
    res.writeHead(200,{
      ...
      'Access-Control-Allow-Origin':'*',
      'X-Accel-Buffering':'no',   // отключает nginx-буферизацию на Railway/Heroku
    ...
    const hb = setInterval(()=>{ try{ res.write(':ping\n\n'); }catch{} }, 10000); // 10s — быстрее чем Railway timeout
